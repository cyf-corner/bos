...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(rows.length != 1) {
			...
			// 发送ajax请求，获取已关联定区的客户---->哪个定区？需要传参所选中行的定区id
	...
	function doDblClickRow(index,data){
		// alert("双击表格数据...");
		// ★★★★★遗留问题，当双击从service获取关联客户数据为null时，浏览器报错，阻止判断，无法正常控制页面元素。
		$('#association_subarea').datagrid( {
			fit : true,
			border : true,
			rownumbers : true,
			striped : true,
			url : "decidedzoneAction_findHasAssociationSubarea.action?id="+data.id,
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
			...
			<form id="addDecidedzoneForm" method="post" action="decidedzoneAction_add.action">
				<table class="table-edit" width="100%" align="center">
					...
					<tr height="300">
						...
						<td>
							<table id="subareaGrid" class="easyui-datagrid" border="true" style="width:480px;height:161px" data-options="url:'subareaAction_listAjax.action',fitColumns:true,singleSelect:false,striped:true,rownumbers:true">
								<thead>
							        <tr>
							            <th data-options="field:'subareaid',width:30,checkbox:true">编号</th>  
							            <th data-options="field:'addresskey',width:150">关键字</th>  
							            <th data-options="field:'position',width:300">位置</th>  
							        </tr>
							    </thead>
							</table>
						</td>
							...
							<script type="text/javascript">
								$(function(){
									...
									$("#associationBtn").click(function(){
										var rows = $("#grid").datagrid("getSelections");
										var id = rows[0].id;
										// 为id为customerDecidedZoneId的隐藏域赋值
										$("#customerDecidedZoneId").val(id);
										// 将下拉框中所有的选项选中，才能通过表单提交
										$("#associationSelect option").attr("selected","selected");
										var flag = $("#associationSelect option").length;
										if(flag == 0){// 此操作为防止select提交表单时，option的值为空，即右边下拉框全部清空导致的错误
											$("#associationSelect").append("<option value='-1' selected='selected'></option>");
										};
										$("#customerForm").submit();
									});
								});
