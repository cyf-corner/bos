...
import top.flyroc.bos.utils.FileUtils;
...
public class SubareaAction extends BaseAction<Subarea> {
	...
	public String pageQuery() throws IOException {
		...
		if (StringUtils.isNotBlank(addresskey)) {// 根据地址模糊
		...
		if (region != null) {
			...
			if (StringUtils.isNotBlank(province)) {// 根据省份模糊
				dc.add(Restrictions.like("r.province", "%" + province + "%"));
			}
			if (StringUtils.isNotBlank(city)) {// 根据市模糊
				dc.add(Restrictions.like("r.city", "%" + city + "%"));
			}
			if (StringUtils.isNotBlank(district)) {// 根据区模糊
	...
	public String add() {
		...
		// 问题二：提交的表单中subareaid参数的值为null		解决方案：在Subarea.java类中添加getSubareaid()方法
